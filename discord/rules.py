...
async def check_rules_channel(bot: disnake.Client, channel_id: int, webhook_config: dict):
    """Проверка и создание сообщения в канале правил"""
    ...
    print(f"♻ Удалено {deleted} сообщений в канале правил")
    ...
    async for message in channel.history(limit=100):
        if (message.author == bot.user or
            ...
            if message.embeds and any("Правила" in embed.title for embed in message.embeds):
                has_message = True
                break

    if has_message:
        print("ℹ Сообщение в канале правил уже существует")
        return
    ...
    try:
        webhook = await channel.create_webhook(
            ...
            reason=webhook_config.get("reason", "Rules message")
        )

        embed = Embed(
            title="Правила",
            description="Правила сервера OmniCorp.",
        ...
        print("✅ Сообщение в канале правил отправлено")

    except Exception as e:
        print(f"⚠ Ошибка при работе с вебхуком в канале правил: {e}")